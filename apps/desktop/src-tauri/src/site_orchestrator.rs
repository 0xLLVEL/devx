...
impl<'a> SiteOrchestrator<'a> {
    ...
    pub async fn remove(
        ...
        if let Some(site) = removed {
            let mut names = vec![site.hostname];
            names.extend(site.aliases);
    ...
    pub async fn delete_alias(
        ...
        // remove hosts entry for alias (best-effort)
        if self.state.with_config(|s| s.config().network.dns_mode) != devx_core::DnsMode::Resolver {
            ...
            if devx_privileged::PipeClient::is_available() {
                if let Ok(mut c) = devx_privileged::PipeClient::connect() {
                    let _ = c.hello().await;
                    let _ = c.remove_hosts_entry(alias).await;
                }
            }
        }
        self.restart_nginx().await;
    ...
    async fn ensure_certs(&self, site: &ConfigSite) {
        ...
        let _ = devx_provision::ensure_site_cert(&self.state.paths.certs_dir(), &site.hostname);
    }
    ...
    async fn restart_nginx(&self) {
        if let Some(sup) = self.state.services.get("nginx") {
            if sup.state().is_active() {
                sup.stop().await;
                let custom = self
                    .state
                    .with_config(|s| s.config().service_ports.get("nginx").copied());
                if let Ok(ver) =
                    crate::commands::sites::newest_installed_component(&self.state.paths, "nginx")
                {
                    if let Ok(plan) =
                        crate::services::plan_service(&self.state.paths, "nginx", &ver, &[], custom)
                    {
